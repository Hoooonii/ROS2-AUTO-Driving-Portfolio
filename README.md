...
2. Edge 환경 AI 추론 최적화
제한된 CPU 자원 내에서 정확도와 속도의 트레이드오프(Trade-off) 분석.

성능 지표	AS-IS (YOLOv8s)	TO-BE (YOLOv8n + 최적화)
추론 시간 (Inference)	62.0 ms (프레임 드랍)	24.5 ms (실시간 처리)
초당 프레임 (FPS)	약 16 FPS (제어 불안정)	약 35 ~ 40 FPS (안정적)
🔥 Issues & Troubleshooting
면접관님을 위한 상세 트러블슈팅 과정입니다. 자세한 내용은 아래 토글을 클릭하여 확인해 주세요.
